...
if(!Titanium.App.Mvc) {
	Titanium.App.Mvc = Mvc;
	
	var getRouteCollection,
		rc; // cached route context -- used to test if we should use the most recent controller
	...
	function loadControllers(context) {
		...
		for(var i = 0, ii = files.length; i < ii; i++) {
			file = files[i];
			if(re.test(file)) {
				context[file.substr(0,10)] = '/app/controllers/' + file;
			}
		}
	}
	
	Mvc.Controllers = {}; // object to hold all controllers -- used in initialization
	Mvc.Views = {}; // object to hold all views
	Mvc.process = function(route) {
		var self = this;
		...
		var rd = r.routeData;
		var cname = rd.controller + "Controller", controller = requires(self.Controllers[cname]);
	...
	Mvc._render = function(route, data, partial, winSettings) {
		var self = this;
	...
	Mvc.mapRoute = function (name, path, defaults) {
	...
	Mvc.start = function (route) { // parameters in case we want to start the app with something other than the default page
