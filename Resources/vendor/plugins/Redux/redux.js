        ...
        parseRJSS: function (file) {
            ...
            if (parsedrjss[file]) {
                return parsedrjss[file];
						...
						file = Titanium.Filesystem.getFile(Titanium.Filesystem.resourcesDirectory + file);
            var rjss = (file.read() + '').replace(/[\r\t\n]/g, ' ');
						// warn(nf.read().text.replace(/[\r\t\n]/g, ' '));
        ...
        includeRJSS: function () {
            for (var i = 0, l = arguments.length; i < l; i++) {
                ...
                try {
                    ...
                    var lineNumber = 2;
                    warn('RJSS "' + arguments[i] + '" has syntax errors.  Review generated code with line numbers => ' + parsedRJSS.replace(/\n/g, function() { return '\n#' + (lineNumber++) + '. ' ; }));
